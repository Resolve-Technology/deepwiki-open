...
- Writes `prompt` to **stdin** (page prompts + RAG context are large; avoids
  ARG_MAX).
- Disables tools/MCP for deterministic pure-text generation (no file/tool side
  effects): `--strict-mcp-config` plus an empty allowed-tools set. Pure
  prompt→markdown; no working directory access required.
...
- **Fails fast** (raises) on: nonzero exit, JSON parse failure, missing
  `result`, or a detected rate-limit / 429 in stderr — matching the existing
  "jobs fail fast with a clear error" philosophy in `llm_dispatch`.
...
CLI nor the subscription credentials. Add read-only mounts:

- Host `~/.local/share/claude` → container (the native ELF binary, v2.1.196),
  with `claude` placed on PATH (symlink or `/usr/local/bin/claude`).
- Host `~/.claude` → `/root/.claude` (Claude Code subscription credentials;
  secrets, cannot be baked into the image).
...
not run on `python:3.11-slim` (glibc mismatch): instead `RUN npm i -g
@anthropic-ai/claude-code` in the Dockerfile and keep only the `~/.claude`
credentials mount. The smoke test (below) determines which path is needed.
...
(`deepwiki_cache_gitlab_poc_code1_cbl_bv401_zh-tw~claude_cli~claude-sonnet-4-6.json`),
so the existing Haiku wiki is left intact.

## Risks / assumptions

- **Premise risk:** if `claude -p` shares the same premium cap that 429s the
  SDK, there is no benefit. The step-0 smoke test catches this cheaply before
  any rebuild/regen work is wasted.
...
- `run_claude_cli` parsing: fake subprocess returning well-formed JSON →
  correct (text, in, out); nonzero exit / bad JSON / rate-limit stderr → raises.
